...
Read these when they are relevant to the current task:

- `CONTEXT.md` at the repo root.
- `docs/adr/`, if it exists.

If an expected file does not exist, proceed silently.
...
If a recommendation or implementation contradicts an existing ADR, surface that conflict explicitly.
