...
| `MCP + Hono` gateway for product tools | `web-interface/gateway/mcp-server.ts` exposes SDK-backed `/mcp`; the old JSON-RPC-shaped `/api/gateway/mcp` route has been removed; `web-interface/gateway/tool-dispatcher.ts` no longer has an intent dispatch entry; SDK tools/list now includes 16 platform tools including screen tools, `memory.preference`, `memory.sensitiveSkip`, and `device.note.write`; MCP calls now carry server-derived subject/device context into OPA/audit | Add policy-gated prepare/commit for high-risk/write tools | `P0` |
