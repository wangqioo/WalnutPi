...
| DB-backed product state | `web-interface/platform/db/schema.ts` defines memory candidate, approved durable memory, sensitive-skip, action approval, audit event, agent turn snapshot, agent turn event, Web session event, and better-auth user/session/account/verification tables; migrations now run through `0007_durable_memory_records.sql`; `memory.preference`, `memory.approve`, and `memory.sensitiveSkip` route through `web-interface/platform/memory/product-state-store.ts`; preference capture creates a candidate only, `memory.approve` writes approved durable memory by candidate id, and sensitive skips store hash/length only; approval prepare/commit routes through `action_approval_records`; gateway audit writes use `audit_events`; active Web session and `/api/agent/turn` ledgers use Postgres with explicit skipped persistence when DB config is absent; better-auth and Mastra runtime storage use Postgres as the main path | Add curated retrieval/pgvector indexing only for approved memory and curated corpus without indexing raw session logs or raw daily notes | `P1` |
...
3. Add curated retrieval/pgvector indexing only for approved memory and curated corpus.
