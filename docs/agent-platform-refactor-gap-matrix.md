...
| DB-backed product state | `web-interface/platform/db/schema.ts` defines memory candidate, sensitive-skip, action approval, audit event, agent turn snapshot, agent turn event, and Web session event tables; migrations now run through `0004_agent_session_ledgers.sql`; `memory.preference` and `memory.sensitiveSkip` route through `web-interface/platform/memory/product-state-store.ts`; sensitive skips store hash/length only; approval prepare/commit routes through `action_approval_records`; gateway audit writes use `audit_events`; active Web session and `/api/agent/turn` ledgers use Postgres with explicit skipped persistence when DB config is absent; Mastra runtime storage uses `@mastra/pg` Postgres-managed `mastra_*` tables | Add approved durable memory and curated retrieval paths without indexing raw session logs | `P1` |
...
| Next.js Walnut Agent Console | `web-interface/next-app/` now has a Tailwind console slice with chat/tool result panels, route/evidence diagnostics, quick Mastra/MCP capabilities, and approval UI for `policy.action.prepare`/`policy.action.commit`; Next rewrites `/api/*` and `/mcp` to the Hono platform server | Move remaining static-console workflows into Next and then retire the static HTML console | `P1` |
...
1. Add real signed-in user flows once the Next.js console owns login/session creation; the current resolver is better-auth-first with a server-derived local-owner control-plane subject when no session exists.
2. Finish Langfuse trace/session correlation for the active turn/tool path beyond the current OTel policy/tool spans.
3. Move Screen Workspace, device diagnostics, and artifact panels into the Next/Tailwind console so the static HTML console can be retired.
