...
| Legacy runtime cleanup | `walnut-ai-terminal/` and `web-interface/action-registry.ts` still carry prototype-era behavior; `/api/agent/turn` no longer calls the local dispatcher as fallback | Delete or narrow remaining prototype surfaces after the platform-owned business paths cover their product duties | `P2` |
...
1. Expand Mastra-owned workflows from the current read-only slice into screen write/sync, memory, diagnostics, and approved action flows without adding parallel local execution paths.
