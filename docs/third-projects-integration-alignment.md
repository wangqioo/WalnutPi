...
当前还没有进入实现阶段。需求确认后，建议第一切片只选一个最小闭环：
...
- 用户不用手写命令，也不用理解编译、manifest 或烧录细节。
- Web 端先能看到要同步的界面。
- 点击“同步到核桃派”后，核桃派真实屏幕最终显示同一界面，允许延迟。
- Web 面向小白只显示同步状态和可理解的失败建议。
- 开发者诊断层能判断当前同步画面对应哪个 `buildId` / artifact hash。
- 失败时开发者能看到第一处错误、日志摘要和设备回证。
- 当前 `walnut screen lvgl`、CLI、SSH 使用方式不被破坏。
