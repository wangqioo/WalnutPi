"""
VAD-based audio monitor.

从 USB UAC 麦克风（ESP32-S3）持续读取 PCM 16kHz 单声道音频，
用 webrtcvad 检测语音边界，把完整的一句话（bytes）回调给调用方。
"""

import audioop
import threading
import time
from typing import Callable, Optional

import sounddevice as sd

try:
    import webrtcvad as _webrtcvad
except ImportError:
    _webrtcvad = None

SAMPLE_RATE = 16000
FRAME_MS = 30
FRAME_SAMPLES = int(SAMPLE_RATE * FRAME_MS / 1000)
FRAME_BYTES = FRAME_SAMPLES * 2
SILENCE_FRAMES = 12
MIN_SPEECH_FRAMES = 4


def find_device(hint) -> Optional[int]:
    """按名称片段查找输入设备，找不到返回 None（使用系统默认）。"""
    if isinstance(hint, int):
        return hint
    if hint and hint != "auto":
        if hint.isdigit():
            return int(hint)
        for i, d in enumerate(sd.query_devices()):
            if hint.lower() in d["name"].lower() and d["max_input_channels"] > 0:
                return i
        return None

    keywords = ["esp32", "voice keyboard", "voicekeyboard", "usb audio", "usb mic"]
    for i, d in enumerate(sd.query_devices()):
        if d["max_input_channels"] > 0:
            name = d["name"].lower()
            if any(k in name for k in keywords):
                return i
    return None


class AudioMonitor:
    """后台持续监听麦克风并在检测到完整句子后回调。"""

    def __init__(
        self,
        on_utterance: Callable[[bytes], None],
        device: Optional[str] = "auto",
        vad_level: int = 2,
    ):
        if _webrtcvad is None:
            raise ImportError(
                "VAD 模式需要 webrtcvad。Python 3.12 及以下：pip install webrtcvad\n"
                "若编译失败，请改用 PTT 模式（config.yaml: audio.mode: ptt）"
            )
        self._on_utterance = on_utterance
        self._device_hint = device
        self._vad = _webrtcvad.Vad(vad_level)
        self._thread: Optional[threading.Thread] = None
        self._stop = threading.Event()
        self._paused = threading.Event()
        self._stream_rate = SAMPLE_RATE
        self._stream_state = None

    def start(self):
        self._stop.clear()
        self._paused.clear()
        self._thread = threading.Thread(target=self._run, daemon=True, name="AudioMonitor")
        self._thread.start()

    def stop(self):
        self._stop.set()

    def pause(self):
        self._paused.set()

    def resume(self):
        self._paused.clear()

    def _run(self):
        device = find_device(self._device_hint)
        info = None
        if device is None:
            print("[audio] 未找到 ESP32 UAC 麦克风，使用系统默认麦克风")
            default_device = sd.default.device[0]
            if default_device is not None and default_device >= 0:
                info = sd.query_devices(default_device)
        else:
            info = sd.query_devices(device)
            print(f"[audio] 使用麦克风: {info['name']}")

        if info is not None:
            default_rate = info.get("default_samplerate") or SAMPLE_RATE
            self._stream_rate = int(round(float(default_rate)))
        else:
            self._stream_rate = SAMPLE_RATE

        stream_blocksize = max(1, int(self._stream_rate * FRAME_MS / 1000))
        print(f"[audio] 采样率: {self._stream_rate} Hz -> {SAMPLE_RATE} Hz")

        raw_buf = bytearray()
        speech_frames = []
        silent_count = 0
        in_speech = False

        def callback(indata, frames, time_info, status):
            chunk = bytes(indata)
            if self._stream_rate != SAMPLE_RATE:
                chunk, self._stream_state = audioop.ratecv(
                    chunk, 2, 1, self._stream_rate, SAMPLE_RATE, self._stream_state
                )
            raw_buf.extend(chunk)

        try:
            with sd.RawInputStream(
                samplerate=self._stream_rate,
                channels=1,
                dtype="int16",
                device=device,
                blocksize=stream_blocksize,
                callback=callback,
            ):
                print("[audio] 开始监听，等待语音...")
                while not self._stop.is_set():
                    if self._paused.is_set():
                        del raw_buf[:]
                        speech_frames.clear()
                        silent_count = 0
                        in_speech = False
                        time.sleep(0.05)
                        continue

                    if len(raw_buf) < FRAME_BYTES:
                        time.sleep(0.005)
                        continue

                    frame = bytes(raw_buf[:FRAME_BYTES])
                    del raw_buf[:FRAME_BYTES]
                    is_speech = self._vad.is_speech(frame, SAMPLE_RATE)

                    if is_speech:
                        in_speech = True
                        silent_count = 0
                        speech_frames.append(frame)
                    elif in_speech:
                        speech_frames.append(frame)
                        silent_count += 1
                        if silent_count >= SILENCE_FRAMES:
                            if len(speech_frames) >= MIN_SPEECH_FRAMES:
                                pcm = b"".join(speech_frames)
                                try:
                                    self._on_utterance(pcm)
                                except Exception as e:
                                    print(f"[audio] STT 回调异常: {e}")
                            speech_frames.clear()
                            silent_count = 0
                            in_speech = False
        except Exception as e:
            print(f"[audio] 麦克风错误: {e}")