...
def frame_to_ascii(frame: np.ndarray, cols: int, rows: int, glyphs: Sequence[str], invert: bool = False) -> str:
...
def write_archive(output: Path, spec: VideoSpec, frames: Iterable[str]) -> None:
    payload = {
        "width": spec.width,
        "height": spec.height,
        "fps": spec.fps,
    ...
    with path.open("rb") as f:
        ...
        while True:
            ...
            if size == 0:
                break
            blob = f.read(size)
            frames.append(gzip.decompress(blob).decode("utf-8"))
    return meta, frames
    ...
    with path.open("rb") as f:
        ...
        while True:
            ...
            if size == 0:
                break
            blob = f.read(size)
            yield None, gzip.decompress(blob).decode("utf-8")
...
def encode_video(input_path: Path, output_path: Path, cols: int, rows: int, fps: float | None, glyph_source: str | None, invert: bool) -> None:
    ...
    try:
        while True:
            ...
            if not ok:
                break
            frames.append(frame_to_ascii(frame, cols, rows, glyphs, invert=invert))
    finally:
        cap.release()
...
def encode_image(input_path: Path, output_path: Path, cols: int, rows: int, glyph_source: str | None, invert: bool) -> None:
    ...
    ascii_frame = frame_to_ascii(frame, cols, rows, glyphs, invert=invert)
    height, width = frame.shape[:2]
    spec = VideoSpec(width=width, height=height, fps=1.0, cols=cols, rows=rows, glyphs=list(glyphs))
    write_archive(output_path, spec, [ascii_frame])


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Encode images or videos into a UTF-8 ASCII archive.")
