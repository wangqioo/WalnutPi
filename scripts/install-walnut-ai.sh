...
#!/bin/sh
set -e
PYTHONPATH=/opt/walnut-ai-video exec python3 -m ai_video.ascii_video.player "$@"
SH
chmod +x /usr/local/bin/walnut-ascii-video
cat > /usr/local/bin/walnut-ascii-video-color <<'SH'
#!/bin/sh
set -e
PYTHONPATH=/opt/walnut-ai-video exec python3 -m ai_video.ascii_video_color.player "$@"
SH
...
PYTHONPATH=/opt/walnut-ai-video python3 -m ai_video.examples.make_demo --out-dir "$out_dir"
