...
export function screenManifestV2EnvelopeHash(manifest) {
  return sha256Hex(stableStringify(normalizeScreenManifestV2(manifest)));
}

export function screenPlaylistV1Hash(playlist) {
  return sha256Hex(stableStringify(normalizeScreenPlaylistV1(playlist)));
}

export async function staticOutputFileSha256(filePath) {
  return sha256Hex(await readFile(filePath));
}

export function rgbaPixelSha256(rgbaPixels, { width = SCREEN_WORKSPACE_WIDTH, height = SCREEN_WORKSPACE_HEIGHT, channels = 4 } = {}) {
  assertPixelBuffer(rgbaPixels, width, height, channels, "rgbaPixels");
  return sha256Hex(Buffer.from(rgbaPixels));
}

export async function rgbaPixelSha256FromImage(filePath) {
...
export function rgb565BufferFromRgba(
  rgbaPixels,
  { width = SCREEN_WORKSPACE_WIDTH, height = SCREEN_WORKSPACE_HEIGHT, channels = 4 } = {},
) {
  assertPixelBuffer(rgbaPixels, width, height, channels, "rgbaPixels");
...
export async function rgb565PixelSha256FromImage(filePath) {
  const { data, info } = await readRgbaPixels(filePath);
  return rgb565PixelSha256FromRgba(data, info);
}

export function animatedOutputSha256(frames) {
...
export function normalizeScreenManifestV2(manifest) {
...
export async function validateScreenManifestV2(manifest, options = {}) {
...
export function normalizeScreenPlaylistV1(playlist) {
...
export async function validateScreenPlaylistV1(playlist, options = {}) {
