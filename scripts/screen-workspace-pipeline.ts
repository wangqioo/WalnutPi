...
import {
  ...
  type AnimatedScreenOutput,
  type ScreenManifestV2,
  type ScreenOutput,
  type ScreenOutputFrame,
...
type ScreenPlanInput = JsonRecord & {
  id?: string;
  screenId?: string;
  outputId?: string;
  title?: string;
...
type ProcessSourceResult = {
  ...
  manifest: ScreenManifestV2;
  output: ScreenOutput;
};
type PlaylistWriteOptions = {
  workspaceRoot?: string;
  manifestId: string;
  durationMs?: number;
  repeat?: number;
...
type StaticProcessResult = {
  manifestOutput: StaticScreenOutput;
  outputJsonPath: string;
  tools: Array<{ name: string; version: string }>;
};
type AnimatedProcessResult = {
  manifestOutput: AnimatedScreenOutput;
  outputJsonPath: string;
  tools: Array<{ name: string; version: string }>;
};
...
export async function processSourceAssetToScreenOutput({
  ...
  now = () => new Date(),
}: ProcessSourceOptions): Promise<ProcessSourceResult> {
...
export async function writeDefaultScreenPlaylist({
  ...
  loop = true,
  playlistId = "default",
}: PlaylistWriteOptions): Promise<PlaylistWriteResult> {
...
async function writeScreenPlaylistItem({
  ...
  playlistId,
  mode,
}: PlaylistWriteOptions & { mode: "replace" | "append" }): Promise<PlaylistWriteResult> {
  const workspace = path.resolve(workspaceRoot);
...
export async function probeMediaWithFfmpeg(sourcePath: string): Promise<{ durationSeconds: number | null; width: number | null; height: number | null; raw: string }> {
...
async function processStaticOutput({ workspace, screenId, sourcePath, preset }: { workspace: string; screenId: string; sourcePath: string; preset: string }): Promise<StaticProcessResult> {
  ...
  const manifestOutput: StaticScreenOutput = {
    type: "static",
...
async function processAnimatedOutput({ workspace, screenId, sourcePath, preset, animation }: { workspace: string; screenId: string; sourcePath: string; preset: string; animation: AnimationBudget }): Promise<AnimatedProcessResult> {
  assertFfmpegAvailable();
  ...
  try {
    ...
    const frames: ScreenOutputFrame[] = [];
    ...
    const manifestOutput: AnimatedScreenOutput = {
      type: "animated",
...
async function renderImagePreset(inputPath: string, outputPath: string, preset: string): Promise<void> {
...
async function readSourceMetadata(filePath: string, sourceAsset: SourceAssetInput) {
  try {
...
function normalizeAnimationBudget(animation: Partial<AnimationBudget>): AnimationBudget {
  return {
...
async function ensureWorkspaceDirs(workspace: string): Promise<void> {
  await Promise.all(
...
function runFfmpeg(args: string[], { allowFailure = false }: { allowFailure?: boolean } = {}): Promise<FfmpegResult> {
...
function cleanSlug(value: any, field: string): string {
...
function cleanText(value: any, field: string, limit: number): string {
...
function cleanInteger(value: any, field: string, low: number, high: number): number {
...
function cleanOptionalIso(value: any): string | null {
