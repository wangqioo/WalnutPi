...
type SshWsData = { child: ChildProcessWithoutNullStreams | null };
type SshWebSocket = { close: () => void; data: SshWsData; send: (chunk: any) => void };

function startSsh(ws: SshWebSocket) {
...
function stopSsh(ws: SshWebSocket) {
...
const server = Bun.serve<SshWsData>({
  hostname: HOST,
  port: PORT,
