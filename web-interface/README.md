...
The manifest home page supports a small beginner-safe status vocabulary: `tone` (`ok`, `warn`, `error`) and `progress` (`0-100`). The current compatibility vocabulary also accepts page-level `components`: `statusCard`, `metricGroup`, `list`, `progress`, `alert`, and `textPage`. These are content components only; the server normalizes them into the fixed four-page LVGL config fields and emits bounded runtime metadata for the device screen, rejects unknown component types, and does not let a manifest request shell commands, SSH, sudo, build behavior, delivery behavior, GPIO output, reboots, flashing, or arbitrary LVGL/C code. Web preview, diagnostic DOM snapshot, generated LVGL config, and LVGL runtime colors all consume the same bounded content.

The screen intent route remains rule-based manifest editing, not arbitrary generation. It can update beginner-facing content fields such as title, subtitle, status, status label/detail, tone, progress, progress label, metrics, lists, alerts, and text-page lines. It does not accept natural-language edits for `schema`, `target`, `source`, page ids, build, SSH, sudo, delivery, or device commands.
