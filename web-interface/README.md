...
The screen intent route can use an OpenAI-compatible `/responses` endpoint, when `OPENAI_API_KEY` is configured, to generate a controlled manifest patch from natural language. The model output is not trusted: the server strips it down to the allowed mutable fields, requires ASCII device text until CJK font support lands, applies the fixed `target` / `source` contract, and runs the shared manifest vocabulary validator before writing. If AI generation is unavailable or invalid, the route falls back to the local rule-based patch generator. It still does not accept natural-language edits for `schema`, `target`, `source`, build, SSH, sudo, delivery, or device commands.
