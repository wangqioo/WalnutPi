...
export function intentTypeToRoute(intent: string, fields: IntentRouteFields = {}): IntentRoute {
  const mapped = {
    ...
    "memory.approve": ["memory.notes", "write"],
    "memory.sensitive_skip": ["memory.notes", "refuse"],
    "policy.action.prepare": ["device.action", "confirm"],
    "policy.action.commit": ["device.action", "confirm"],
  ...
  const route: IntentRoute = {
    ...
    exposure: intent?.startsWith("policy.") ? ["internal", "agent_action"] : readOnlyIntent(intent) ? ["internal", "agent_action"] : ["internal"],
    actionPolicyId: null,
    parameters: {},
