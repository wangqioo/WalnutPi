...
import { createAgentTurnLoop, runAgentTurn, selectTurnPlan, selectTurnStep } from "./agent-turn-loop.ts";
...
assert.deepEqual(Object.keys(replanned.turn.loop.turns[0]), ["sourceStepId", "observation", "judgment", "proposal", "policy", "autoContinuedTasks", "blockedTasks"]);
assert.equal(replanned.turn.loop.turns[0].proposal.source, "action");
assert.equal(replanned.turn.loop.turns[0].policy.accepted, true);
...
const modelBackedReplan = await runAgentTurn({
  body: {
    text: "优化现在的 agent loop 和 harness 之前，先做一次只读观察；如果模型判断可以续步，只允许自动做 status 只读检查，其他都阻塞",
    sessionId: "model-replan-demo",
    ...
    scenario: {
      ...
      goal: "model-backed-system-loop-replan",
      constraints: ["read-only-continuation", "model-proposal-required", "system-loop-optimization"],
      requiredEvidence: ["replan-evidence", "multi-step-loop", "agent-loop"],
  ...
  runAction: async (body) => ({
    ...
    body: {
      ...
      nextTasks: body.action === "snapshot" ? [{ agent: "device", kind: "action.run", action: "status" }] : [],
    },
  }),
  loopModelAdapter: {
    propose: async (_context, options) => ({
      proposal: {
        ...
        blockedTasks: [],
        evidencePlan: [{ evidenceId: "agent-loop", blocking: true }],
      },
      diagnostics: {
        provider: "fixture",
        model: options.model,
        ...
        rawOutputHash: "fixture-raw",
        validationErrors: [],
      },
    }),
  },
});
assert.equal(modelBackedReplan.status, 200);
assert.equal(modelBackedReplan.turn.input.loopModel.model, "gpt-5.5");
assert.equal(modelBackedReplan.turn.input.loopModel.reasoningEffort, "high");
assert.equal(modelBackedReplan.turn.loop.turns[0].proposal.source, "model");
assert.equal(modelBackedReplan.turn.loop.turns[0].proposal.kind, "continue");
...
const oracleLeak = await runAgentTurn({
  body: {
    text: "查状态",
    sessionId: "oracle-leak-demo",
    oracle: { goal: { route: "device.action" } },
  },
  classifyIntent: async () => {
    throw new Error("oracle leak should fail before classification");
  },
});
assert.equal(oracleLeak.status, 400);
assert.match(oracleLeak.turn.error, /oracle is harness-only/);

const scenarioVeto = await runAgentTurn({
  body: {
    text: "观察后只允许 status 自动续步",
    sessionId: "scenario-demo",
    scenario: {
      schema: "walnutpi.loopScenario.v1",
      goal: "observation-replan",
      constraints: ["read-only-continuation"],
      requiredEvidence: ["replan-evidence"],
      allowedContinuations: [{ kind: "action.run", action: "status" }],
      blockedPolicy: { requiresConfirmation: ["device-write", "screen-sync", "service-restart", "package-install", "reboot"] },
    },
  },
  ...
  runAction: async () => ({
    ...
    body: {
      ...
      title: "观察结果",
      output: "ran snapshot",
      safeAutoContinue: [{ agent: "device", kind: "action.run", action: "network" }],
    },
  }),
});
assert.equal(scenarioVeto.status, 200);
assert.equal(scenarioVeto.turn.pendingNext.reason, "scenario-continuation-not-allowed");
assert.equal(scenarioVeto.turn.loop.turns[0].vetoes[0].blockedBy, "loop-policy");
assert.equal(scenarioVeto.turn.loop.turns[0].vetoes[0].agentProposed, "auto");
assert.equal(scenarioVeto.turn.evidence.some((item) => item.kind === "scenario-contract"), true);
assert.equal(scenarioVeto.turn.evidence.some((item) => item.kind === "scenario-contract-result"), true);
assert.equal(scenarioVeto.turn.evidence.find((item) => item.kind === "scenario-contract-result").value.loopStatus, "blocked");
...
assert.deepEqual(overBudgetReplan.turn.loop.turns[0].blockedTasks, [{ agent: "device", kind: "action.run", action: "network", reason: "max-continuation-tasks", blockedBy: "loop-policy", agentProposed: "auto" }]);
