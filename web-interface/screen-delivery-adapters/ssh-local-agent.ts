...
function buildActivationCheckCommand() {
  return [
    ...
    "if [ \"$screen_state\" != active ]; then printf 'hot-reload failed: walnut-screen.service %s\\n' \"$screen_state\"; else printf 'hot-reload wait complete\\nwalnut-screen.service %s\\n' \"$screen_state\"; fi",
...
function parseRemoteSyncStages(syncResult): StageResults {
...
function missingStageResult(syncResult, stage) {
  return {
    ok: false,
...
function skippedStageResult(output): StageResult {
...
function summarizeRemoteExecution(commandResults) {
  ...
  for (const result of Object.values(commandResults || {})) {
    ...
    const stage = result as Record<string, any>;
    if (stage.remoteTransport) transports.push(stage.remoteTransport);
    if (typeof stage.reusedConnection === "boolean") reused.push(stage.reusedConnection);
  }
  return {
...
function skippedResult({ sliceResult, validateResult, buildResult, artifactHashValid, activationOk = true, stateOk = true, screenName = "screen slice", missingStage = null, activationLabel = null }: Record<string, any>): StageResult {
  let output = "skipped";
