...
export async function loadActionPolicyManifest(manifestPath) {
  const manifest = JSON.parse(await readFile(manifestPath, "utf8"));
  return normalizeActionPolicyManifest(manifest);
}

export function normalizeActionPolicyManifest(manifest) {
...
export function actionsForExecutor(manifest, executor) {
...
function normalizeAllowedExecutors(value, field) {
